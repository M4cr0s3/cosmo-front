...
import ULoading from "./Loading.vue";
...
<template>
  <div class="mt-[20px]">
        ...
        <a @click.prevent="showSong(song.id)" class="flex items-center text-indigo-700">
          <div class="flex w-full justify-between dark:bg-indigo-950 items-center rounded-lg shadow-md px-10 py-3 border-b
            ...
            <div class="listens flex items-center">
              ...
              {{
                song.watches > 1000 ? Math.round((song.watches /
    ...
    <div class="w-full flex justify-center" v-if="songs && !loading && pagination.current_page !== 10 && !error">
    ...
    <ULoading v-else-if="songs.length !== 100 && !error"/>
